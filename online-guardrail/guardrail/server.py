...
from guardrail.mock_evaluator import mock_evaluate
from guardrail.trajectory import extract_tool_list, extract_trajectory_text
from guardrail.bridge import (
    build_metrics,
    cascade_from_session_events,
    get_trajectory,
    guardrail_check,
    list_trajectories,
)
from guardrail.finegrained import evaluate_finegrained
from guardrail.xai_bridge import attribute_trajectory, load_sample_attribution, xai_scripts_available
...
class GuardrailHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/trajectories":
            self._json_response(200, list_trajectories())
            return
        if path.startswith("/api/trajectories/"):
            trajectory_id = path.removeprefix("/api/trajectories/").strip("/")
            trajectory = get_trajectory(trajectory_id)
            if trajectory is None:
                self._json_response(404, {"error": "trajectory not found"})
                return
            self._json_response(200, trajectory)
            return
        if path == "/api/metrics":
            self._json_response(200, build_metrics())
            return
        if path == "/api/l1/rules":
            from guardrail.l1_rules import list_rules

            self._json_response(200, list_rules())
            return
        if path == "/api/l1/codeshield/status":
            from guardrail.codeshield_bridge import codeshield_available

            self._json_response(200, codeshield_available())
            return
        if path == "/api/xai/status":
            self._json_response(
                200,
                {
                    "agentdog_repo": str(Path(__file__).resolve().parents[2] / "AgentDoG"),
                    "xai_scripts": xai_scripts_available(),
                    "mock": os.environ.get("GUARDRAIL_MOCK", "0"),
                },
            )
            return
    ...
    def do_POST(self) -> None:
        if self.path == "/api/guardrail/check":
            body = self._read_body()
            if body is None:
                return
            self._json_response(200, guardrail_check(body, self.config))
            return
        if self.path == "/api/cascade/check":
            body = self._read_body()
            if body is None:
                return
            session_events = body.get("session_events", "")
            self._json_response(200, cascade_from_session_events(session_events, self.config))
            return
        if self.path == "/api/agentdog/finegrained":
            body = self._read_body()
            if body is None:
                return
            session_events = body.get("session_events", "")
            if not session_events and body.get("accumulatedSteps"):
                from guardrail.bridge import _steps_to_session_events

                session_events = _steps_to_session_events(body["accumulatedSteps"])
            self._json_response(200, evaluate_finegrained(session_events, self.config))
            return
        if self.path == "/api/xai/attribute":
            body = self._read_body()
            if body is None:
                return
            steps = body.get("accumulatedSteps") or []
            step_index = body.get("stepIndex")
            if not steps and body.get("use_sample"):
                sample = load_sample_attribution(body.get("sample", "exfil-chain"))
                self._json_response(200, sample or {"error": "sample not found"})
                return
...
def _evaluate_trajectory(
    ...
    if os.environ.get("GUARDRAIL_MOCK", "").lower() in {"1", "true", "yes"} or (
        not config.api_key or not config.api_base
    ):
        mock = mock_evaluate(trajectory, config.model)
        return {
            "prediction": mock.prediction,
            "label": _prediction_label(mock.prediction),
            "reason": mock.reason,
            "model": mock.model,
            "error": mock.error,
        }
